...
## Prefer the official examples' compact summaries

For a straightforward clip, one natural sentence for the soundscape and one for the score will often suffice; this is a starting point, not a word limit. Add sentences only to preserve a meaningful source relationship, user requirement or audible change that would otherwise be lost. Detailed dialogue and complex sound-led scenes may need more space.

The soundscape identifies the dominant environment and distinguishing sounds, not every possible noise in that environment. The score identifies its characteristic instrumentation and rhythmic feel, adding development only where needed. Translate mood into audible features rather than explaining what the audience should feel. Do not expand a sentence with wind, hiss, hum, fabric noise or extra bass simply to make it sound complete.

Place precise local cues in their shots. Summaries may name these sounds collectively but should not replay their sequence, contact details and stop conditions. Keep a globally important silence or source-track boundary clear without repeating its full instructions. Do not merely move the removed summary detail into the shots: delete optional details that contribute no distinct audible information.

For example, when the requested treatment is a firelit VHS fashion montage:

`overall_soundscape: Fire roars and crackles beneath brief analog interference synchronized with the broadcast glitches.`

`non_diegetic_music: A fast synth-wave instrumental with a pulsing bassline and clipped drum-machine beats.`

These are illustrative wording choices, not a universal soundtrack. Retain any user-required footsteps, special sound or ending in its appropriate place. A quieter material study, a conversation and a source-music edit require different content, not the same sentence with substituted nouns.

When a visual plan is supplied as fixed, audio planning must not redesign its scenes, actions, cut times or transitions. For a fresh joint generation, this instruction does not guarantee identical visual outputs across runs.

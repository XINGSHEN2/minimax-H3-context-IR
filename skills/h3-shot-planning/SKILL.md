...
同一时间、地点、主体和动作目标，先尝试用一个连续镜头拍清楚。人物运动、摄影机跟随、重新构图、景别变化或焦点变化能把准备、动作、细节、即时反应和结果接起来时，留在同一镜头。一个 Shot 可以包含多个连续的动作阶段，不必固定一个景别。

只有换镜后才能看清当前镜头无法呈现的重要内容或结果，时间、地点或主要观察对象确实改变，或者用户、参考明确要求此处切换时，才增加 Shot。快节奏、硬切和电影感决定已有切点怎样表现，不单独增加切点。用户明确要求蒙太奇、舞蹈卡点或独立展示时可以按要求切换，但每个镜头仍要呈现新阶段、新对象或新信息。标题变化、闪白、黑场、故障和普通反应通常附在已有镜头或切点上；只有明确要求独立片段时才单独成镜。

列出初步镜头后，逐个尝试合并相邻两镜：合并后，用户要求的内容或重要结果会不会看不清？若仍能通过主体或摄影机运动、重新构图、景别或焦点变化看清，就合并。只多一个角度、再次展示同一标题材质，或把同一次震动拆成几个景别，不足以保留切点。合并会遮住必需信息，或违反用户、参考指定的结构时，才保留。镜头数由内容决定，不设固定数量。

## 第三步：直接写每个镜头

在 `[Shot N]` 正文中依次写清：开头先看见谁和什么，它们在哪里、处于什么状态；人物或物体接下来怎样运动、状态怎样改变；用户要求的材质、结构、文字或表情在什么画面和光线下看清；摄影机怎样跟随或重新构图；结束时留下什么画面和状态。不要只写“展示产品”“人物反应”或一个景别名称，让后续步骤猜镜头内容。
...
逐对检查相邻镜头。前镜结束时，人物做到了哪一步、朝哪里运动、物品由谁持有；后镜从这个状态继续，不重做动作、不无故换位或恢复旧画面。服装、环境和需要延续的声音也要接上。同一动作确实需要拆镜时，在后镜的 `cut_reason` 写明前镜为什么看不清必要内容，在 `continuity_bridge` 写明从哪个动作阶段接入。
...
提交前检查四件事：用户要求和 developments 是否都拍到了；每个切点合并后是否真会损失重要内容；每镜的画面、动作和摄影机是否具体可拍；前后镜的动作、物品、关键帧、声音和转场是否接得上。锁定后交给 Sound Planning Skill；后续只排版，不重新设计镜头。

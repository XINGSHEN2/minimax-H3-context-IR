...
调用方负责解释用户要求、提供素材证据、规定输出字段和阶段顺序。本 Skill 负责从允许补全的范围内选择内容发展，判断镜头应合并或拆分，并把锁定的计划落实为可执行镜头。不要增加额外模型调用、审查关卡或默认镜头数量。

## 内容发展

一个 development 必须带来可见的动作、关系、空间、信息或结果变化。标题动画、角度、景别、显示介质、运镜、转场、特效、声音和风格只属于表现层，不能单独充当主要事件。
...
“快切、蒙太奇、紧凑剪辑、广告节奏”等全局风格只决定已有必要切点怎样发生，不能自行决定镜头数量或创建切点。只有用户给出具体镜头序列、明确要求某处切换，或参考结构被要求严格迁移时，剪辑风格才锁定对应切点。

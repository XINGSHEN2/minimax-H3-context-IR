...
VERSION='glm53_two_stage_stream_nothink_v2'
...
def run(n):
 p=BASE/f'case{n}-ref2va/output'/VERSION;p.mkdir();local.folder=p;local.calls=0;t=time.time();m={'version':VERSION,'case':n,'status':'running','output':str(p),'model':'sha/GLM-5.3','stream':True,'thinking':False,'max_tokens_per_call':16384,'compiler_revision':COMPILER_REVISION,'reused_perception':True,'workflow':'two_streaming_requests_frozen_shot_plan_then_h3'};save(out/f'case{n}.json',m)
 try:
  ...
  plan_prompt=build_shot_plan_prompt(e);(p/'shot_plan_instructions.txt').write_text(plan_prompt)
  planned=invoke_reasoning_json(plan_prompt,config,p/'shot_planner.log',['h3-shot-planning']);save(p/'shot_plan_result.json',planned)
  plan=planned.get('content_plan');assert isinstance(plan,dict),'planner returned no content_plan';save(p/'frozen_content_plan.json',plan)
  ...
  j={'content_plan':plan,'h3_prompt':compiled.get('h3_prompt'),'uncertainties':planned.get('uncertainties',[])};save(p/'compilation_result.json',j);errors,warnings=transport_issues(j,e);save(p/'h3_prompt_audit.json',{'errors':errors,'warnings':warnings,'semantic_quality_verified':False,'llm_calls':2})

...
Use the official `h3-prompt-writing` Skill for H3 prompt semantics and the internal `h3-shot-planning` Skill for camera movement, shot functions, cuts, timing, and editorial continuity. The shot-planning Skill may complete missing technical coverage conservatively, but it must never override explicit user shots, asset authority, entity truth, or H3 output structure.

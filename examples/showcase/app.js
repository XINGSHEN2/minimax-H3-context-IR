const state={data:null,caseIndex:0,variant:'original',generation:0},$=s=>document.querySelector(s),videos=()=>[...document.querySelectorAll('video')];
...
async function render(){
 ...
 for(const [,group]of entries){
  ...
  if(group.video){const video=node('video');video.controls=true;video.muted=true;video.playsInline=true;video.preload='metadata';video.loop=$('#loopAll').checked;video.src=group.video;card.append(video)}else{const missing=node('div','尚未生成');missing.className='missing';card.append(missing)}
  ...
  const details=node('details'),pre=node('pre','加载中…');details.append(node('summary','查看 H3 Prompt'),pre);card.append(details);

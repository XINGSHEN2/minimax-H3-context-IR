...
# Load file-based runtime configuration without committing
...
docker exec \
  ...
  -e YIWU_VLM_BASE_URL="${YIWU_VLM_BASE_URL:-http://10.6.157.43:9012}" \
  -e QWEN_IMAGE_UNDERSTAND_BASE_URL="${QWEN_IMAGE_UNDERSTAND_BASE_URL:-http://10.6.157.43:9012}" \
  -e QWEN_VIDEO_UNDERSTAND_BASE_URL="${QWEN_VIDEO_UNDERSTAND_BASE_URL:-http://10.6.157.43:9012}" \
  -e QWEN_ASSET_UPLOAD_BASE_URL="${QWEN_ASSET_UPLOAD_BASE_URL:-http://10.0.96.114:30100}" \
  -e YIWU_VLM_MODEL="${YIWU_VLM_MODEL:-Qwen3.8-27B}" \
  ...
  -e CONTEXT_IR_LLM_RUNTIME=direct \
  -e CONTEXT_IR_LLM_CHAT_BASE_URL \
  -e DEEPSEEK_LITELLM_CHAT_BASE_URL \
  -e GLM_CHAT_BASE_URL \
  -e DEEPSEEK_CHAT_BASE_URL \
  -e CONTEXT_IR_LLM_TIMEOUT_SECONDS \
  -e CONTEXT_IR_LLM_PROVIDER="${CONTEXT_IR_LLM_PROVIDER:-deepseek}" \
  ...
  -e CONTEXT_IR_DEEPSEEK_REASONING_EFFORT="${CONTEXT_IR_DEEPSEEK_REASONING_EFFORT:-}" \
...
# The existing runtime image runs as root. Return only this project's generated/cache files

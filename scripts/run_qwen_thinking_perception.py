...
class RecordingThinkingProvider(LocalQwen3VL32BProvider):
    ...
    def _request_json(self, method: str, path: str, payload: Mapping[str, Any] | None=None, timeout: float=30.0, base_url: str | None=None) -> dict[str, Any]:
        ...
        with self.response_lock:
            ...
            (self.response_dir/f"{stem}.final.txt").write_text(final_content+"\n",encoding="utf-8")
        return response
...
def main() -> int:
    ...
    config=PerceptionProviderConfig(provider="local-qwen3-vl-32b",model="Qwen3.8-27B",options={"image_base_url":os.environ["QWEN_IMAGE_UNDERSTAND_BASE_URL"],"video_base_url":os.environ["QWEN_VIDEO_UNDERSTAND_BASE_URL"],"asset_upload_base_url":os.environ["QWEN_ASSET_UPLOAD_BASE_URL"],"output_dir":str(output_dir/"qwen_work"),"cache_enabled":False,"max_parallel_assets":0,"single_pass_image_analysis":True,"single_pass_video_analysis":True,"max_tokens":6000,"relational_image_max_tokens":6000,"video_single_pass_max_tokens":6000,"enable_thinking":True,"temperature":0.0,"timeout_seconds":7200,"video_fps":2.0,"video_max_frames":256})

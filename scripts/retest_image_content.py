"""Single fresh image inference with the original intent-derived inspection plan."""
import json
import os
...
base = ROOT / 'outputs/feishu_case3_authority_timing_20260905'
out = ROOT / 'outputs/feishu_case3_image_content_20260907'
...
try:
    ...
    print(json.dumps({'seconds': time.perf_counter()-start, 'result': result}, ensure_ascii=False), flush=True)
finally:
    (out / 'timing.json').write_text(json.dumps({'seconds': time.perf_counter()-start, 'cache': False, 'json_retries': 0}))

...
3. 视觉分镜锁定后，按 H3 Sound Planning Skill 形成 audio_plan 与必要的 shots.sound_cues。声音不得改变事件、镜头、动作、时间、运镜或结尾画面。
4. 按 H3 Prompt Writing Skill 将锁定计划编译为完整六节 h3_prompt。写作阶段只做信息分配、去重复和可执行表达，不重新设计大纲、分镜或声音。
5. 最终核对用户要求覆盖、真实素材编号、时间连续性和计划到 H3 的一致性。问题必须回到所属 Skill 修正；content_plan 只保存最终锁定版本。
"""

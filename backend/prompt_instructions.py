...
12. 最终检查：要求分层是否正确且全部落实；developments 是否只含内容变化并单向推进；所有非用户明示的事件是否通过删除测试；每镜是否有非风格性的 content_purpose；去风格测试后是否仍有新内容；镜头是否为最小充分数量；动作、方向、空间、持物和隐藏式替换是否连续；分镜、音频和最终 H3 中的每个新动作与事件是否都能映射到锁定的 development；音频是否保持视觉分镜锁定。发现问题直接修正最终 requirement_map、developments、shots 和 H3，content_plan 只保存最终版。"""

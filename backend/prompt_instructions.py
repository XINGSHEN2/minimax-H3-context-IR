...
第二阶段：根据大纲安排 shots
4. 先选择主要剪辑语法和任务覆盖策略，不套用固定走路动作、镜头数或时长比例：
- 叙事/动作：观看目的改变时切镜。
...
5. 生成初步分镜。每镜用 development_ids 指向大纲，并记录连续时间、start_state、action、end_state 和 sound_cues。新镜头只有在其观看价值大于重新建立主体、空间和动作的连续性成本时成立。观看价值是当前镜头无法充分呈现的重要动作、关系、尺度、时间地点、产品功能或独立特征、结果、情绪距离，或用户指定节奏；换角度、换景别、增强光效/震动、普通反应和再次确认结果本身不构成价值。开放创作允许有节制的视角层次，但不机械遍历景别或机位。

连续动作压缩
6. 完成初步分镜后，按时间顺序检查相邻镜头。若它们处于连续时间和同一空间、共享同一主体与动作目标，并且下一镜的信息能够通过当前镜头内的主体运动、摄影机移动、构图调整、景别渐变或焦点变化清楚呈现，则合并为一个镜头。合并后的镜头可以包含多个有序阶段，不要求保持固定景别。产品细节优先在拿取、使用、转动或移动过程中显露；人物反应优先与触发动作保持在同一连续段中；动作结果优先在动作结束时直接呈现。
仅在时间/空间改变、观察对象改变、当前构图无法显示必要结果或独立展示维度、用户明确要求切换，或剪辑节奏本身承担明确意义时保留切镜。若删除两镜之间的切换描述后能自然连接，就默认合并；合并会遮蔽关键信息、破坏空间方向或违反用户/参考结构时保留。一个镜头可以完成多个 developments，一个 development 也可跨必要镜头，但后镜须提供非重叠价值并承接已达动作阶段。

连续性与边界
7. 跨切镜续接动作阶段、方向、观察侧、相对位置和物品归属；需要流动感时在动作进行中切出并从未完成阶段切入，不停住或重演。只检查必要前提，如持物手、握持和交接；检查即时因果反应，不插入无关节拍，不为收尾新增姿势或动作。每个切点只选一个主要连接依据：动作、视线、因果、构图、声音或有意跳跃。
8. 模糊、闪白、烟雾、暗场或遮挡用于替换素材/场景时，把边界写成一个不可逆过程：“旧内容完全遮蔽 → 遮蔽峰值完成替换 → 只显露新内容”；替换后旧内容不得重新清晰。风格和特效修饰内容或边界，不独立授权事件或镜头。

第三阶段：写 H3 并核对
9. 将最终方案实际落实到 h3_prompt，使用随附 H3 skill 的格式。一次响应内修正开放部分，确保 developments、shots 和 H3 是同一版。
10. 最终只检查五项：
...
- detailed_description：全局风格和持续效果写一次；镜头写动作、关系、构图、摄影和动态变化，不重述固定外观。通用转场定义一次。文字需完整处于画面内并留边距；用户明确要求裁切字形或局部显露时按其时段执行。
...
content_plan 包含：creative_brief（user_locked、reference_anchors、open_design）；task_mode；must_keep；bindings（asset_id、role、retained_attributes、optional_inherited_attributes、excluded_attributes、exclusion_reasons）；developments（id、visible_change、outcome）；shots（development_ids、start_seconds、end_seconds、start_state、action、end_state、sound_cues）。
